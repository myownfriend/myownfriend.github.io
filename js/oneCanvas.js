...
window.background = gl.canvas.appendChild(Object.assign(document.createElementNS(namespace, 'symbol'), {
	...
	set: (file) => {
		...
		bg.image.onload = analyzeBackground, { once: true };

		analyst.onmessage = updateBackground;
		...
		function updateBackground(e) {
			...
			addAnimation(300, updateLights, pruneOldBackground);
			addAnimation(300, update);
		}
	},
	cleanup : pruneOldBackground,
}));

function pruneOldBackground() {
...
function draw(element) {
	if (element.hasOwnProperty('depth')) {
		...
		gl.uniform1f(radius, Number(getComputedStyle(element).
	...
	if (element.deadline > performance.now() && element.hasOwnProperty('cleanup')) {
		element.cleanup();
	}
}